...
void DSCSPlugin::FetchStats()
{
	...
	this->Log(("Map: " + gameWrapper->GetCurrentMap()).c_str());
	...
	if (socket.opened() > 0) {
		...
		socket.socket()->emit("match_ended", "Score: Bleu (" + std::to_string(teams.Get(0).GetScore()) + ") - Orange (" + std::to_string(teams.Get(1).GetScore()) + ")");
	}
